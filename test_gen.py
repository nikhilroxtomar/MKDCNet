...
if __name__ == "__main__":
    ...
    """ MedAI - Mix """
    path = "/media/nikhil/Seagate Backup Plus Drive/ML_DATASET/MedAI-challenge-final-data-22september"

    normal_test_x = sorted(glob(os.path.join(path, "normal", "images", "*")))
    normal_test_y = sorted(glob(os.path.join(path, "normal", "normal_mask", "*")))

    polyp_test_x = sorted(glob(os.path.join(path, "polyp-mediai-test", "images", "*")))
    polyp_test_y = sorted(glob(os.path.join(path, "polyp-mediai-test", "masks", "*")))

    test_x = polyp_test_x + normal_test_x
    test_y = polyp_test_y + normal_test_y

    print(f"Images: {len(test_x)} - Masks: {len(test_y)}")

    save_path = f"results/MedAI-Mix"
    for item in ["mask", "joint"]:
        create_dir(f"{save_path}/{item}")
    ...
    save_path = f"results/MedAI-Polyp"
    for item in ["mask", "joint"]:
        create_dir(f"{save_path}/{item}")
